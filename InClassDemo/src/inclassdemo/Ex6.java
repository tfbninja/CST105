...
public class Ex6 {
    ...
    public static boolean isPalindromic(int x, boolean debug) {
        x = Math.abs(x); // support for negative numbers
        int numDigits = (int) (Math.floor(Math.log10(x)) + 1.0); // the number of digits is equal to the floor( of the base10 log of a number + 1)
        if (debug) {
            System.out.println("\tnumDigits in " + x + ": " + numDigits);
            System.out.println("\t\tCalculated by taking the log base 10 of " + x + " which returns " + Math.log10(x) + " then flooring that which makes " + Math.floor(Math.log10(x)) + " then adding 1 to finally give " + (Math.floor(Math.log10(x)) + 1) + ".");
        }
        for (int i = 0; i < Math.floor(numDigits / 2); i++) { // loop through the outer two digits, moving inwards as we go, ignoring a middle number if there is one
            int leftDigit = (int) (Math.floor(x % Math.pow(10, numDigits - i) / Math.pow(10, numDigits - (i + 1)))); // i'd rather not share how long these two lines took to perfect
            int rightDigit = (int) (Math.floor(x % Math.pow(10, (i + 1)) / Math.pow(10, i)));
            if (leftDigit != rightDigit) { // if the outer digits are not the same, it's not a palindrome
                if (debug) {
                    System.out.println("\tDigits " + (i + 1) + " and " + (numDigits - i) + " from the left in " + x + " which are " + leftDigit + " and " + rightDigit + " respectively, are NOT considered matching.");
                    System.out.println("\ti is " + i);
                }
                return false;
            }
            if (debug) {
                System.out.println("\tDigits " + (i + 1) + " and " + (numDigits - i) + " from the left in " + x + " which are " + leftDigit + " and " + rightDigit + " respectively, are considered matching.");
                System.out.println("\ti is " + i);
            }
        }
        return true;
    ...
    public static boolean isPrime(int x) {
        for (int i = 2; i < x; i++) { // starting at 2, no prime number can be equally divised by any positive integer except itself
            if (x % i == 0) { // if the number is equally divised...
                return false; // it ain't prime
            }
        }
        if (x > 1 && x / x == 1) { // double check that the number is positive and divisible by itself (order is important, otherwise will error on 0)
